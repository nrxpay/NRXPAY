...
const SavingsAccountManagement = () => {
  ...
  const fetchSavingsAccounts = async () => {
    ...
    try {
      ...
      const accountsWithUsernames = savingsData.map(account => {
  ...
  const removeRechargePopup = async (accountId: string) => {
    console.log('Attempting to remove popup for savings account:', accountId);
    ...
    try {
      ...
      if (data && data.length > 0) {
        ...
        console.log('Successfully removed popup for account:', accountId);
      } else {
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Savings Account Management</CardTitle>
        <CardDescription>
          Manage and verify savings account applications
        </CardDescription>
      </CardHeader>
      <CardContent>
        <div className="space-y-4">
          ...
          <div className="space-y-4">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                  {selectedAccount.aadhar_photo_url && (
                                    <div>
                                      ...
                                      <img 
                                        src={`${supabase.storage.from('savings-docs').getPublicUrl(selectedAccount.aadhar_photo_url).data.publicUrl}`}
                                        alt="Aadhar Document"
                                  ...
                                  {selectedAccount.pan_photo_url && (
                                    <div>
                                      ...
                                      <img 
                                        src={`${supabase.storage.from('savings-docs').getPublicUrl(selectedAccount.pan_photo_url).data.publicUrl}`}
                                        alt="PAN Document"
                                        className="w-full h-32 object-cover border rounded-lg mt-1"
                                        onError={(e) => {
                                          e.currentTarget.style.display = 'none';
                      ...
                      {account.status === 'approved' && (
                        <div className="flex items-center gap-2">
                          <div className="flex items-center gap-2 bg-yellow-50 border border-yellow-200 px-3 py-2 rounded-lg">
                            ...
                            <span className="text-sm font-medium text-yellow-700">Needs Recharge ($300)</span>
                          </div>
                          <Button
                            size="sm"
            ...
            {filteredAccounts.length === 0 && (
              <p className="text-center text-muted-foreground py-8">No savings accounts found</p>
            )}
          </div>
        </div>
      </CardContent>
